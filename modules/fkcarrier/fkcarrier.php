...
class fkcarrier extends CarrierModule {
	...
	public function install() {
		...
		if (!parent::install()
			...
			Or !Configuration::updateValue('FKCARRIER_REFERENCIA', '')
            Or !Configuration::updateValue('FKCARRIER_DOMINIO', '')
            Or !Configuration::updateValue('FKCARRIER_PROPRIETARIO', '')
			...
			Or !Configuration::updateValue('FKCARRIER_CUSTOS_ENVIO', '0')
			Or !Configuration::updateValue('FKCARRIER_EMBALAGEM', '0')
	...
	private function postValidation() {
		...
		switch($sessao) {
			...
			case 'configRegistro_2':
				...
				if (Tools::isSubmit('submitSave')) {
					if (!$this->wsAlteraLicenca(Configuration::get('FKCARRIER_REFERENCIA'), Tools::getShopDomain(false,true))) {
	...
	private function validaRegistro_1($sessao) {
		...
		// Recupera valores
		$referencia = Trim(Tools::getValue('fkcarrier_referencia'));
		$dominio = Trim(Tools::getShopDomain(false,true));
		$proprietario = Trim(Tools::getValue('fkcarrier_proprietario'));
	...
	private function validaGeral($sessao) {
		...
		if (Tools::getValue('fkcarrier_custos_envio') == NULL) {
			$this->_postErrors[] = $this->l('Custos de envio não preenchido');
		}else {
			$valor = str_replace(',', '.', Tools::getValue('fkcarrier_custos_envio'));
	
			if (!is_numeric($valor)) {
				$this->_postErrors[] = $this->l('O campo "Custos de envio" não é numérico');
			}
            else {
                if ($valor < 0) {
                    $this->_postErrors[] = $this->l('O campo "Custos de envio" não pode ser menor que 0 (zero)');
                }
            }
		}
	...
	private function salvaRegistro_1() {
		...
		Configuration::updateValue('FKCARRIER_REFERENCIA', Trim(Tools::getValue('fkcarrier_referencia')));
		Configuration::updateValue('FKCARRIER_DOMINIO', Trim(Tools::getShopDomain(false,true)));
		Configuration::updateValue('FKCARRIER_PROPRIETARIO', Trim(Tools::getValue('fkcarrier_proprietario')));
	}
	
	private function salvaRegistro_2() {
		
		Configuration::updateValue('FKCARRIER_REFERENCIA', '');
		Configuration::updateValue('FKCARRIER_DOMINIO', '');
		Configuration::updateValue('FKCARRIER_PROPRIETARIO', '');
	}
	...
	private function salvaGeral() {
		...
		$custo_envio = str_replace(',', '.', Tools::getValue('fkcarrier_custos_envio'));
		Configuration::updateValue('FKCARRIER_CUSTOS_ENVIO', $custo_envio);
	...
	private function salvaEmbalagens() {
		...
		$embalagens = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'fkcarrier_embalagens`');
	...
	private function displayForm() {
		...
		if (Configuration::get('FKCARRIER_REFERENCIA') == '' Or Configuration::get('FKCARRIER_DOMINIO') == '' Or Configuration::get('FKCARRIER_PROPRIETARIO') == '') {
	...
	private function displayFormConfig() {
		...
		if (Configuration::get('FKCARRIER_REFERENCIA') != '' and Configuration::get('FKCARRIER_DOMINIO') != '' and Configuration::get('FKCARRIER_PROPRIETARIO') != '') {
			 
			if (!$this->wsVerificaLicenca(Configuration::get('FKCARRIER_REFERENCIA'), Tools::getShopDomain(false,true))) {
				...
				$this->_html .= '   <li id="menuTab8" class="menuTabButton">8. '.$id_transp.'</li>';
                $this->_html .= '   <li id="menuTab9" class="menuTabButton">9. '.$id_frete_gratis.'</li>';
                $this->_html .= '   <li id="menuTab10" class="menuTabButton">10. '.$id_tabelas_off.'</li>';
				$this->_html .= '</ul>';
		
				$this->_html .= '<div id="fkcarrier_tabList">';
		
				$this->_html .= '   <div id="menuTab1Sheet" class="fkcarrier_tabItem selected">';
				ob_start();
				...
				$this->_html .= '   <div id="menuTab1Sheet" class="fkcarrier_tabItem">';
				ob_start();
	...
	public function getOrderShippingCost($params, $shipping_cost) {
        ...
        $fkcarrier = array(
            ...
            'custos_envio'                      => Configuration::get('FKCARRIER_CUSTOS_ENVIO'),
    ...
    public function hookdisplayShoppingCartFooter($params) {
        ...
        $fkcarrier_cep_msg = array('msg' => $this->l('Aguardando CEP'));
        $this->context->smarty->assign(array('fkcarrier_cep_msg' => $fkcarrier_cep_msg));

        $fkcarrier_cep_frete = array('frete' => array());

        // Calcula o frete
        if (Tools::isSubmit('submitCarrinho') Or $this->context->customer->isLogged() Or isset($this->context->cookie->fkcarrier_cep)){
            ...
            if (strlen($cep_destino) <> 8) {
                ...
                $fkcarrier_cep_msg['msg'] = $this->l('CEP inválido');
                $this->context->smarty->assign(array('fkcarrier_cep_msg' => $fkcarrier_cep_msg));
            ...
            // Recupera dados dos servicos dos Correios e Transportadoras
            ...
            foreach ($correios_transp as $reg) {
                ...
                $fkcarrier_cep_msg['msg'] = $this->l('Frete calculado');
                $this->context->smarty->assign(array('fkcarrier_cep_msg' => $fkcarrier_cep_msg));
                ...
                $fkcarrier_cep_frete['frete'][] = array(
            ...
            usort($fkcarrier_cep_frete['frete'], array($this, 'ordenaValorFrete'));

            $this->context->smarty->assign(array('fkcarrier_cep_frete' => $fkcarrier_cep_frete));
    ...
    private function processaCorreiosOnline($fkcarrier) {
        ...
        if ($total_frete > 0) {
            return $total_frete + $fkcarrier['custos_envio'];
        }else {
            return $total_frete;
        }
