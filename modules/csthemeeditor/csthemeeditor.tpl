...
if(!isMobile())
{
	jQuery(document).ready(function($) {
		{foreach from=$patterns item=pattern name=patterns}
			...
			$("#b_{$id_pattern[0]}").click(function() {
				$(".pattern_item").removeClass("active");
				$(this).addClass("active");
				var url_pattern = "{$path}/images/general/patterns/{$pattern}";
				$.cookie('cookie_bg_pattern',url_pattern);
				$.cookie('cookie_bg_pattern_class','{$id_pattern[0]}');
		...
		<ul id="improved">
			...
			<li>
				...
				<div class="content" style="display: none;">
					{foreach from=$patterns item=pattern name=patterns}
						{assign var=id_pattern value="."|explode:$pattern}
						<div class="pattern_item" id="b_{$id_pattern[0]}" style="background:url({$path}/images/general/patterns/{$pattern})"></div>
					{/foreach}
				</div>
			</li>
			<li>
				...
				<div class="content">
				{foreach from=$colors key=elem_color item=color name=colors}
					<div class="color_item">
					<h4>{$color[0]}</h4>
					<div id="{$elem_color}" class="bg_color_setting" style="cursor:pointer">text</div>
					<div class="explain">{$color[1]}</div>
					</div>
				{/foreach}
				</div>
			</li>
			<li>
				...
				<div class="content">
					...
					<div class="font_item">
					<h4>{$font[0]}</h4>
					<select name="{$elem_font}" id="{$elem_font}" onchange="showResultChooseFont('{$elem_font}','result_{$elem_font}')">
					{$font_list}
					</select>
					<span id="result_{$elem_font}"></span>
					<div class="explain">{$font[1]}</div>
					</div>
				{/foreach}
				</div>
			</li>
