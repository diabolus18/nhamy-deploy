...
class csblockbestsellers extends Module
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('CS Top seller block');
	...
	public function install()
	{
		if (!parent::install() ||
				...
				!Configuration::updateValue('CS_LEFT_BESTSELLERS_NUMBER',1) ||
				!Configuration::updateValue('CS_HOME_BESTSELLERS_NUMBER',5))
			return false;
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitBestSellers'))
		{
			...
			Configuration::updateValue('CS_LEFT_BESTSELLERS_NUMBER', (int)(Tools::getValue('left_number')));
			Configuration::updateValue('CS_HOME_BESTSELLERS_NUMBER', (int)(Tools::getValue('home_number')));
	...
	public function displayForm()
	{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset>
				...
				<label>'.$this->l('Number product display left, right column').'</label>
				<div class="margin-form">
				<input type="input" name="left_number" value="'.Configuration::get('CS_LEFT_BESTSELLERS_NUMBER').'" />
				</div><br/>
				<label>'.$this->l('Number product display home column').'</label>
				<div class="margin-form">
				<input type="input" name="home_number" value="'.Configuration::get('CS_HOME_BESTSELLERS_NUMBER').'" />
				</div>
	...
	public function hookRightColumn($params)
	{
		...
		$bestsellers = ProductSale::getBestSalesLight((int)($params['cookie']->id_lang), 0, Configuration::get('CS_LEFT_BESTSELLERS_NUMBER'));
		...
		return $this->display(__FILE__, 'csblockbestsellers.tpl');
	}
	...
	public function hookHeader($params)
	{
		...
		{
			$this->context->controller->addCSS(($this->_path).'css/csblockbestsellers.css', 'all');
	...
	public function hookHome($params)
	{
		...
		$currency = new Currency($params['cookie']->id_currency);
		$bestsellers = ProductSale::getBestSalesLight((int)$params['cookie']->id_lang, 0, Configuration::get('CS_HOME_BESTSELLERS_NUMBER'));
		if (!$bestsellers && !Configuration::get('PS_BLOCK_BESTSELLERS_DISPLAY'))
			return;
		$best_sellers = array();
		...
		return $this->display(__FILE__, 'csblockbestsellers-home.tpl');
	}
